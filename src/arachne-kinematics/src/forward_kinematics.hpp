...
void leg2base(Matrix4d& T0, const VectorXd& base, const int& leg) {
    Matrix3d rotm;
    ...
    q2rot(rotm, base(3), base(4), base(5));
    trans << rotm(0, 0), rotm(0, 1), rotm(0, 2), base(0),
             rotm(1, 0), rotm(1, 1), rotm(1, 2), base(1),
             rotm(2, 0), rotm(2, 1), rotm(2, 2), base(2),
             0, 0, 0, 1;
    T0 = trans * Tbase;
}

void forward_kinematics(Vector3d& pos, const Vector3d& q, const VectorXd& base, const int& leg) {
    ...
    leg2base(T0, base, leg);

    // Denavit-Hartenberg matrices
    dh(T1,            q(0), pi/2,      5.1, -0.5925);
